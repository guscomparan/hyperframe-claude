...
description: Asset preprocessing for HyperFrames compositions — text-to-speech narration (Kokoro), audio/video transcription (ElevenLabs Scribe — the only speech transcriber; Whisper is banned), and background removal for transparent overlays (u2net). Use when generating voiceover from text, transcribing speech for captions, removing the background from a video or image to use as a transparent overlay, choosing a TTS voice, or chaining these (TTS → transcribe → captions). Each command downloads its own model on first run.
---
...
> **Project standard: transcribe SPEECH with ElevenLabs Scribe — NOT the Whisper `transcribe`
> command.** Whisper mangles proper nouns/brand names (Claude→"Cloth", DeepSeek→"Dipsick"), so
> in this project it is banned for transcribing audio/video. Use
> `.claude/skills/tiktok-studio/scripts/transcribe_elevenlabs.py <src> <out_dir> [lang]`
> (needs `ELEVENLABS_API_KEY`). The CLI `transcribe` below is kept ONLY for **importing existing
> subtitle files** (`.srt`/`.vtt`/`openai-response.json`), never for generating a transcript from speech.

Produce a normalized `transcript.json` with word-level timestamps.

```bash
# Speech → transcript (REQUIRED path):
python3 .claude/skills/tiktok-studio/scripts/transcribe_elevenlabs.py audio.mp3 out_dir es

# CLI transcribe — import existing subtitles only (NOT speech-to-text):
...
> Applies only to the legacy Whisper path, which is **banned for speech here** (use ElevenLabs
> Scribe with its `language_code`, e.g. `spa`/`eng`). Kept for reference.
...
python3 .claude/skills/tiktok-studio/scripts/transcribe_elevenlabs.py narration.wav . eng   # → transcript.json
```

ElevenLabs Scribe extracts precise word boundaries from the generated audio, so caption timing matches delivery without hand-tuning. (Project standard — not Whisper; see the Transcription banner above.)
