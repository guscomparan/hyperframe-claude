...
- **Band height = the hairline, measured per video — never hardcode.** Run
  `python3 edit/detect_hairline.py [cut.mp4]` → `edit/hairline.txt`; `build_comp.py` reads it.
...
## Captions (karaoke, inverted house style)
...
- Inverted style — no box, black fill + white outline:
  ```css
  .cgroup .cbg { display:inline-block; max-width:920px; text-align:center; }   /* no background */
  .cgroup .w {
    display:inline-block; padding:0 4px; color:#000;
    -webkit-text-stroke:5px #fff; paint-order:stroke fill;     /* white outline behind black */
    text-shadow:0 2px 6px rgba(0,0,0,.35);
  }
  ```
- Active (karaoke) word: `tl.set(ws,{color:ACCENT,scale:1.05})` → green fill, white outline;
  revert to `color:"#000000"`. (Previous style = black pill + white text + green highlight;
  in git history.)
...
<audio id="bg-music" src="music/background-music.mp3" ... data-track-index="6" data-volume="0.251"></audio> <!-- -12 dB -->
...
```
MUSIC/MUSIC_DB come from `config.py`. `volume = 10^(dB/20)`: −12 dB → 0.251, −18 dB → 0.126. Music spans full `TOTAL` and hard-cuts
at the end. `TOTAL` is read from `edit/edl.json` so it tracks the cut automatically.
